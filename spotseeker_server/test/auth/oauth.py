...
class SpotAuthOAuth(TestCase):
    ...
    def test_put_no_oauth(self):
        ...
        # Test PUT with the spot form set in settings.py
        ...
        # Additionally test with the default form if there's an org specific one
        if settings.SPOTSEEKER_SPOT_FORM:
            with self.settings(SPOTSEEKER_SPOT_FORM='spotseeker_server.default_forms.spot.DefaultSpotForm'):
                # Get the spot again so we've got an up-to-date ETag
                response = c.get(self.url)
                etag = self.spot.etag
                response = c.put(self.url, json.dumps(spot_dict), content_type="application/json", If_Match=etag)
                self.assertEquals(response.status_code, 401, "Rejects a PUT w/o oauth info")
