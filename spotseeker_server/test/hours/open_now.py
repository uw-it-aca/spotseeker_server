...
class SpotHoursOpenNowTest(TestCase):
    ...
    def test_open_now(self, datetime_mock):
        ...
        open_hours = SpotAvailableHours.objects.create(
            spot=open_spot, day=today, start_time=open_start, end_time=open_end
        )
        ...
        self.assertEquals(
            has_closed_spot, False, "Doesn't find the closed spot"
        )
        self.assertEquals(
            has_no_hours_spot, False, "Doesn't find the spot with no hours"
        )
    ...
    def test_thirty_sec_before_midnight(self, datetime_mock):
        ...
        open_hours = SpotAvailableHours.objects.create(
            spot=open_spot,
            day="m",
            start_time=datetime(12, 3, 12, 18, 00, 00).time(),
            end_time=datetime(12, 3, 12, 23, 59, 00).time(),
        )
        open_hours = SpotAvailableHours.objects.create(
            spot=open_spot,
            day="t",
            start_time=datetime(12, 3, 12, 00, 00, 00).time(),
            end_time=datetime(12, 3, 12, 6, 00, 00).time(),
        )
        ...
        open_hours = SpotAvailableHours.objects.create(
            ...
            day="t",
            start_time=datetime(12, 3, 12, 00, 00, 00).time(),
            end_time=datetime(12, 3, 12, 18, 00, 00).time(),
        )
        ...
        datetime_mock.return_value = (
            "m",
            datetime(12, 3, 12, 23, 58, 00).time(),
        )
        response = c.get("/api/v1/spot", {"open_now": True})
        self.assertEqual(response.status_code, 200)
