...
class ImageThumbTest(ImageTestCase):
        ...
        with self.settings(MEDIA_ROOT=self.TEMP_DIR):
            ...
            for content_type, img_format, image_name in image_formats:
                ...
                for width, height in good_dimensions:
                    ...
                    im = Image.open(data)
                    self.assertEquals(
                        im.format, img_format, "img format did not match"
                    )
                    self.assertEquals(
                        im.size[0],
                        width,
    ...
    def test_constrain_jpeg(self):
        self._test_aspect_ratio(
            "image/jpeg", "JPEG", "%s/test_jpeg2.jpg" % image_folder
        )

    def test_constrain_png(self):
        self._test_aspect_ratio(
            "image/png", "PNG", "%s/test_png2.png" % image_folder
        )

    def test_constrain_gif(self):
        self._test_aspect_ratio(
            "image/gif", "GIF", "%s/test_gif2.gif" % image_folder
        )
    ...
    def _test_aspect_ratio(self, content_type, img_format, img_file):
        with self.settings(MEDIA_ROOT=self.TEMP_DIR):
            c = self.client
            response = self.upload_image(
                img_file, self.url, {"description": "test %s #2" % img_format}
            )
