...
class ItemImagePOSTTest(TestCase):
    ...
    def setUp(self):
        ...
        item = Item.objects.create(
            name="This is to test adding images", spot=spot
        )
        item.save()
        self.spot = spot
        self.item = item
    ...
    def test_display_index(self):
        with self.settings(MEDIA_ROOT=self.TEMP_DIR):
            ...
            self.assertEquals(
                item_dict["images"].__len__(), 1, "Item has only 1 ItemImage"
            )
            self.assertEquals(
                item_dict["images"][0]["display_index"],
                1,
    ...
    def test_no_display_index(self):
        with self.settings(MEDIA_ROOT=self.TEMP_DIR):
            ...
            item_dict = item_dict["items"][0]
            self.assertEquals(
                item_dict["images"].__len__(), 2, "Item has 2 ItemImages"
            )
            self.assertEquals(
                item_dict["images"][0]["display_index"],
                0,
            ...
            self.assertEquals(
                item_dict["images"][1]["display_index"],
                1,
