...
class SpotPOSTTest(TestCase):
    ...
    def test_valid_json(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            new_capacity = 10
            response = c.post('/api/v1/spot/', '{"name":"%s","capacity":"%d", "location": {"latitude": 50, "longitude": -30} }' % (
                new_name, new_capacity), content_type="application/json", follow=False)
            ...
            self.assertEquals(response["Location"], 'http://testserver' +
                              self.spot.rest_url(), "The uri for the new spot is correct")
            ...
            self.assertEquals(spot_json["name"],
                              new_name, "The right name was stored")
            self.assertEquals(
                spot_json["capacity"], new_capacity, "The right capacity was stored")
    ...
    def test_extended_info(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            new_name = "testing POST name: {0}".format(random.random())
            new_capacity = 10
            json_string = '{"name":"%s","capacity":"%s", "location": {"latitude": 50, "longitude": -30},"extended_info":{"has_outlets":"true"}}' % (
            ...
            self.assertEquals(
                spot_json["extended_info"], extended_info, "extended_info was succesffuly POSTed")
    ...
    def test_multiple_correct_extended_info(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            c = Client()
            json_string1 = '{"name":"%s","capacity":"10", "location": {"latitude": 50, "longitude": -30},"extended_info":{"has_whiteboards":"true", "has_printing":"true", "has_displays":"true", "num_computers":"38", "has_natural_light":"true"}}' % (
                "testing POST name: {0}".format(random.random()))
            response1 = c.post('/api/v1/spot/', json_string1,
                               content_type="application/json", follow=False)
            json_string2 = '{"name":"%s","capacity":"10", "location": {"latitude": 50, "longitude": -30},"extended_info":{"has_outlets":"true", "has_outlets":"true", "has_scanner":"true", "has_projector":"true", "has_computers":"true"}}' % (
                "testing POST name: {0}".format(random.random()))
            response2 = c.post('/api/v1/spot/', json_string2,
                               content_type="application/json", follow=False)
            json_string3 = '{"name":"%s","capacity":"10", "location": {"latitude": 50, "longitude": -30},"extended_info":{"has_outlets":"true", "has_printing":"true", "has_projector":"true", "num_computers":"15", "has_computers":"true", "has_natural_light":"true"}}' % (
                "testing POST name: {0}".format(random.random()))
            ...
            self.assertEquals(spot_json1["extended_info"], json.loads(json_string1)[
                              'extended_info'], "extended_info was succesffuly POSTed")
            self.assertEquals(spot_json2["extended_info"], json.loads(json_string2)[
                              'extended_info'], "extended_info was succesffuly POSTed")
            self.assertEquals(spot_json3["extended_info"], json.loads(json_string3)[
                              'extended_info'], "extended_info was succesffuly POSTed")
