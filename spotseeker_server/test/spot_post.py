...
class SpotPOSTTest(TestCase):
    ...
    def test_valid_json(self):
        ...
        self.assertEquals(
            response.status_code, 201, "Spot did not get created"
        )
        self.assertIn(
            "Location", response, "Did not find Location header in response"
        )
    ...
    def test_create_spot_with_bad_items(self):
        ...
        bad_spot_json = (
            ...
            '"subcategory" : "itemsubcategory"}] }' % (new_name, new_capacity)
        )
    ...
    def test_item_extended_info(self):
        ...
        for key, value in zip(ei_keys, ei_values):
            self.assertIn(
                key,
                json_response["items"][0]["extended_info"],
