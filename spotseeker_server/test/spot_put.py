...
class SpotPUTTest(TestCase):
    ...
    def test_bad_json(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            c = Client()
            response = c.put(self.url, 'this is just text',
                             content_type="application/json", If_Match=self.spot.etag)
    ...
    def test_empty_json(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            c = Client()
            response = c.put(
                self.url, '{}', content_type="application/json", If_Match=self.spot.etag)
    ...
    def test_valid_json_no_etag(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            new_capacity = 10
            response = c.put(self.url, '{"name":"%s","capacity":"%d", "location": {"latitude": 55, "longitude": 30} }' % (
                new_name, new_capacity), content_type="application/json")
            self.assertEquals(response.status_code, 400,
                              "Bad request w/o an etag")

            updated_spot = Spot.objects.get(pk=self.spot.pk)
            self.assertEquals(updated_spot.name,
                              self.spot.name, "No etag - same name")
            self.assertEquals(updated_spot.capacity,
                              self.spot.capacity, "no etag - same capacity")
    ...
    def test_valid_json_valid_etag(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            response = c.put(self.url, '{"name":"%s","capacity":"%d", "location": {"latitude": 55, "longitude": 30} }' % (
                new_name, new_capacity), content_type="application/json", If_Match=etag)
    ...
    def test_valid_json_outdated_etag(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            self.assertEquals(updated_spot.capacity, intermediate_spot.capacity,
    ...
    def test_no_duplicate_extended_info(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            response = c.get(self.url)
            etag = response["ETag"]
            response = c.put(self.url, '{"name":"%s","capacity":"%d", "location": {"latitude": 55, "longitude": 30}, "extended_info": {"has_a_funky_beat": "true"} }' % (
                new_name, new_capacity), content_type="application/json", If_Match=etag)
            ...
            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="has_a_funky_beat")), 1, 'Only has 1 has_a_funky_beat SpotExtendedInfo object after 2 PUTs')

            response = c.get(self.url)
            etag = response["ETag"]
            response = c.put(self.url, '{"name":"%s","capacity":"%d", "location": {"latitude": 55, "longitude": 30}, "extended_info": {"has_a_funky_beat": "of_course"} }' % (
                new_name, new_capacity), content_type="application/json", If_Match=etag)

            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="has_a_funky_beat")), 1, 'Only has 1 has_a_funky_beat SpotExtendedInfo object after 3 PUTs')
            self.assertEquals(self.spot.spotextendedinfo_set.get(key="has_a_funky_beat").value,
                              'of_course', 'SpotExtendedInfo was updated to the latest value on put.')
    ...
    def test_deleted_spot_info(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            response = c.get(self.url)
            etag = response["ETag"]
            response = c.put(self.url, '{"name":"%s","capacity":"%d", "location": {"latitude": 55, "longitude": 30}, "extended_info": {"eats_corn_pops": "true", "rages_hard": "true", "a": "true", "b": "true", "c": "true", "d": "true"} }' % (
                new_name, new_capacity), content_type="application/json", If_Match=etag)

            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="eats_corn_pops")), 1, 'Has 1 eats_corn_pops SpotExtendedInfo object after 1 PUT')
            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="rages_hard")), 1, 'Has 1 rages_hard SpotExtendedInfo object after 1 PUT')
            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="a")), 1, 'Has 1 a SpotExtendedInfo object after 1 PUT')
            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="b")), 1, 'Has 1 b SpotExtendedInfo object after 1 PUT')

            response = c.get(self.url)
            etag = response["ETag"]
            response = c.put(self.url, '{"name":"%s","capacity":"%d", "location": {"latitude": 55, "longitude": 30}, "extended_info": {"eats_corn_pops": "true"} }' % (
                new_name, new_capacity), content_type="application/json", If_Match=etag)

            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="eats_corn_pops")), 1, 'Has 1 eats_corn_pops SpotExtendedInfo object after a PUT')
            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="rages_hard")), 0, 'Has 0 rages_hard SpotExtendedInfo object after a PUT and a DELETE')
            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="a")), 0, 'Has 0 a SpotExtendedInfo object after 1 PUT')
            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="b")), 0, 'Has 0 b SpotExtendedInfo object after 1 PUT')
            ...
            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="eats_corn_pops")), 0, 'Has 0 eats_corn_pops SpotExtendedInfo object after a PUT and a DELETE')
            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="rages_hard")), 0, 'Has 0 rages_hard SpotExtendedInfo object after a PUT and a DELETE')
            self.assertEquals(len(self.spot.spotextendedinfo_set.filter(
                key="a")), 0, 'Has 0 a SpotExtendedInfo object after 1 PUT')
            ...
            response = c.get(self.url)
            etag = response["ETag"]
            response = c.put(self.url, '{"name":"%s","capacity":"%d", "manager":"Yoda", "location": {"latitude": 55, "longitude": 30, "building_name": "Coruscant"} }' % (
                new_name, new_capacity), content_type="application/json", If_Match=etag)

            self.assertEquals(len(Spot.objects.get(
                name=new_name).manager), 4, 'Has 1 manager object after a PUT')
            self.assertEquals(len(Spot.objects.get(
                name=new_name).building_name), 9, 'Has 1 building_name object after a PUT')
            ...
            self.assertEquals(len(Spot.objects.get(
                name=new_name).manager), 0, 'Has 0 manager objects after a PUT and a DELETE')
            self.assertEquals(len(Spot.objects.get(name=new_name).building_name),
                              0, 'Has 0 building_name objects after a PUT and a delete')
    ...
    def test_extended_info(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            new_name = "testing PUT name: {0}".format(random.random())
            new_capacity = 20
            json_string = '{"name":"%s","capacity":"%s", "location": {"latitude": 55, "longitude": 30}, "extended_info":{"has_outlets":"true"}}' % (
            ...
            self.assertEquals(
                spot_json["extended_info"], extended_info, "extended_info was successfully PUT")
    ...
    def test_new_extended_info(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            self.assertEqual(spot_json1["extended_info"], json.loads(new_json_string1)[
                             'extended_info'], "extended_info was succesffuly PUT")
            self.assertContains(new_response1, '"something_new": "true"')
            self.assertEqual(spot_json2["extended_info"], json.loads(new_json_string2)[
                             'extended_info'], "extended_info was succesffuly PUT")
            self.assertContains(new_response2, '"another_new": "yep it is"')
            self.assertEqual(spot_json3["extended_info"], json.loads(new_json_string3)[
                             'extended_info'], "extended_info was succesffuly PUT")
