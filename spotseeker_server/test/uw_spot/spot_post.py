...
class UWSpotPOSTTest(TestCase):
    ...
    def test_valid_json(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            new_capacity = 10
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_whiteboards":"1","has_outlets":"0","manager":"Bob"}}' % (new_name, new_capacity)
    ...
    def test_uw_field_has_whiteboards(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            new_capacity = 10
            whiteboards = 12
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_whiteboards":"%s","has_outlets":"1","manager":"John"}}' % (new_name, new_capacity, whiteboards)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_has_outlets(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_whiteboards":"0","manager":"Harry"}}' % (new_name, new_capacity)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
            self.assertEquals(response.status_code, 400, "Not created because has_outlets was not included")

            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_whiteboards":"0","has_outlets":"%s","manager":"Harry"}}' % (new_name, new_capacity, outlets)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_printer_nearby(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            new_capacity = 10
            printer = 12
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"1","printer_nearby":"%s","manager":"Gary"}}' % (new_name, new_capacity, printer)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_scanner_nearby(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"1","scanner_nearby":"%s","manager":"Sally"}}' % (new_name, new_capacity, scanner)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_has_displays(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"1","has_displays":"%s","manager":"Fred"}}' % (new_name, new_capacity, has_displays)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_has_projector(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"1","has_projector":"%s","manager":"George"}}' % (new_name, new_capacity, has_projector)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_computers(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"1","computers":"%s","manager":"Tina"}}' % (new_name, new_capacity, computers)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_has_natural_light(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"1","has_natural_light":"%s","manager":"Mary"}}' % (new_name, new_capacity, has_natural_light)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_noise_level(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"1","noise_level":"%s","manager":"Patty"}}' % (new_name, new_capacity, noise_level)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_food_nearby(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            new_capacity = 10
            food_nearby = 'The ave'
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"1","food_nearby":"%s","manager":"Kristy"}}' % (new_name, new_capacity, food_nearby)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_manager(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            new_name = "testing POST name: {0}".format(random.random())
            new_capacity = 10
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"1"}}' % (new_name, new_capacity)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)

            self.assertEquals(response.status_code, 400, "Not created because manager field was not included")

            manager = 'vegitron'
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"1","manager":"%s"}}' % (new_name, new_capacity, manager)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
