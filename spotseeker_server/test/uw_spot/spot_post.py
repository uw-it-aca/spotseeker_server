...
class UWSpotPOSTTest(TestCase):
    ...
    def test_valid_json(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            new_capacity = 10
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_whiteboards":"true","has_outlets":"false","manager":"Bob","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity)
    ...
    def test_uw_field_has_whiteboards(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            new_capacity = 10
            whiteboards = 12
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_whiteboards":"%s","has_outlets":"true","manager":"John","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, whiteboards)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_has_outlets(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_whiteboards":"false","manager":"Harry","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
            self.assertEquals(response.status_code, 400, "Not created because has_outlets was not included")

            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_whiteboards":"false","has_outlets":"%s","manager":"Harry","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, outlets)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_printer_nearby(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            new_capacity = 10
            printer = 12
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","printer_nearby":"%s","manager":"Gary","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, printer)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_scanner_nearby(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","scanner_nearby":"%s","manager":"Sally","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, scanner)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_has_displays(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","has_displays":"%s","manager":"Fred","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, has_displays)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_has_projector(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","has_projector":"%s","manager":"George","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, has_projector)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_computers(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","computers":"%s","manager":"Tina","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, computers)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_has_natural_light(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","has_natural_light":"%s","manager":"Mary","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, has_natural_light)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_noise_level(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","noise_level":"%s","manager":"Patty","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, noise_level)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_food_nearby(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","food_nearby":"%s","manager":"Kristy","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, food_nearby)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_manager(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
                           SPOTSEEKER_SPOT_FORM='spotseeker_server.org_forms.uw_spot.UWSpotForm'):
            c = Client()
            new_name = "testing POST name: {0}".format(random.random())
            new_capacity = 10
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)

            self.assertEquals(response.status_code, 400, "Not created because manager field was not included")

            manager = 'vegitron'
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","manager":"%s","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, manager)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)

            self.assertEquals(response.status_code, 201, "Gives a Created response to creating a Spot")

    def test_uw_field_organization(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
                           SPOTSEEKER_SPOT_FORM='spotseeker_server.org_forms.uw_spot.UWSpotForm'):
            c = Client()
            new_name = "testing POST name: {0}".format(random.random())
            new_capacity = 10
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","manager":"Ken","ada_accessible":"true"}}' % (new_name, new_capacity)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","organization":"%s","manager":"Ken","ada_accessible":"true"}}' % (new_name, new_capacity, organization)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_ada_accessible(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_whiteboards":"false","has_outlets":"true","ada_accessible":"%s","manager":"Harry","organization":"UW"}}' % (new_name, new_capacity, ada_accessible)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
    ...
    def test_uw_field_reservable(self):
        with self.settings(SPOTSEEKER_AUTH_MODULE='spotseeker_server.auth.all_ok',
            ...
            new_capacity = 10
            reservable = 'You bet'
            json_string = '{"name":"%s","capacity":"%s","extended_info":{"has_outlets":"true","reservable":"%s","manager":"Patty","organization":"UW","ada_accessible":"true"}}' % (new_name, new_capacity, reservable)
            response = c.post('/api/v1/spot/', json_string, content_type="application/json", follow=False)
