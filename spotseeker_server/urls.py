...
urlpatterns = [
    url(r'v1/null$', csrf_exempt(NullView.as_view())),
    url(r'v1/spot/(?P<spot_id>(\d+|external:[\w-]+))$',
        csrf_exempt(SpotView.as_view()), name='spot'),
    url(r'v1/spot/?$',
        csrf_exempt(SearchView.as_view()),
        name='spot-search'),
    url(r'v1/spot/all$',
        csrf_exempt(AllSpotsView.as_view()),
        name='spots'),
    url(r'v1/buildings/?$',
        csrf_exempt(BuildingListView.as_view()),
        name='buildings'),
    url(r'v1/schema$',
        csrf_exempt(SchemaGenView.as_view()),
        name='schema'),
    url(r'v1/spot/(?P<spot_id>\d+)/image$',
        csrf_exempt(AddImageView.as_view())),
    url(r'v1/spot/(?P<spot_id>\d+)/image/'
        r'(?P<image_id>\d+)$',
        csrf_exempt(ImageView.as_view()),
        name='spot-image'),
    url(r'v1/spot/(?P<spot_id>\d+)/image/'
        r'(?P<image_id>\d+)/thumb/constrain/'
        '(?P<thumb_dimensions>.+)?$',
        csrf_exempt(ThumbnailView.as_view()),
        {'constrain': True}),
    url(r'v1/spot/(?P<spot_id>\d+)/image/'
    ...
    url(r'v1/item/(?P<item_id>\d+)/image$',
        csrf_exempt(AddItemImageView.as_view())),
    url(r'v1/item/(?P<item_id>\d+)/image/'
        r'(?P<image_id>[\d]+)$',
    ...
    url(r'v1/item/(?P<item_id>\d+)/image/'
        r'(?P<image_id>\d+)/thumb/constrain/'
        r'(?P<thumb_dimensions>.+)?$',
        csrf_exempt(ItemThumbnailView.as_view()),
        {'constrain': True}),
    url(r'v1/item/(?P<item_id>\d+)/image/'
        r'(?P<image_id>\d+)/thumb/'
        r'(?P<thumb_dimensions>.+)?$',
    ...
    url(r'v1/spot/(?P<spot_id>\d+)/reviews$',
        csrf_exempt(ReviewsView.as_view())),
    url(r'v1/user/me/favorites/?$',
        csrf_exempt(FavoritesView.as_view())),
    url(r'v1/user/me$',
        csrf_exempt(PersonView.as_view())),
    url(r'v1/user/me/favorite/(?P<spot_id>\d+)$',
        csrf_exempt(FavoritesView.as_view())),
    url(r'v1/spot/(?P<spot_id>\d+)/share$',
        csrf_exempt(ShareSpaceView.as_view())),
    url(r'v1/spot/(?P<spot_id>\d+)/shared$',
        csrf_exempt(SharedSpaceReferenceView.as_view())),
    url(r'v1/reviews/unpublished$',
        csrf_exempt(UnpublishedReviewsView.as_view())),
]
