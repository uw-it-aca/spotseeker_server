...
class ShareSpaceView(RESTDispatch):
    ...
    def PUT(self, request, spot_id):
        ...
        try:
            share = SharedSpace.objects.get(
                space=spot, sender=send_from, user=user.username
            )
        ...
        for to in send_to:
            try:
                server = getattr(
                    settings, "SS_APP_SERVER", socket.gethostname()
                )
                path = getattr(
                    settings,
                    "SS_APP_SPACE_PATH",
                ...
                path = re.sub(
                    r"{{\s*spot_name\s*}}", urlquote(spot.name), path
                )
                ...
                try:
                    ...
                    recipient = SharedSpaceRecipient(
                        shared_space=share,
                        hash_key=hash_val,
                        recipient=to,
                        shared_count=1,
                ...
                try:
                    location_description = SpotExtendedInfo.objects.get(
                        spot=spot, key="location_description"
                    ).value
                except ObjectDoesNotExist:
                    pass
                ...
                raise RESTException(
                    "Cannot share space at this time: %s" % (e),
                    status_code=500,
                )
