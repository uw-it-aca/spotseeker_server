...
class UWSearchTest(TestCase):
    ...
    def setUp(self):
        ...
        self.ei2 = SpotExtendedInfo.objects.create(spot=self.spot2, key="app_type", value="food")
        # create a spot with some other app_type
        self.spot3 = Spot.objects.create(name="resource checkout spot", capacity=4)
        self.ei3 = SpotExtendedInfo.objects.create(spot=self.spot3, key="app_type", value="checkout")
    ...
    def test_app_type(self):
        ...
        response = self.client.get("/api/v1/spot", {"capacity":4, "extended_info:app_type":"food"})
    ...
    @skipUnless(getattr(settings, 'SPOTSEEKER_SEARCH_FILTERS', False) and settings.SPOTSEEKER_SEARCH_FILTERS == ['spotseeker_server.org_filters.uw_search.Filter'], "Skip unless the right search filter is defined in settings")
